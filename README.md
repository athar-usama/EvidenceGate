...
A held-out calibration sweep asked a blunt question first: *what precision can this
evidence actually support?* An ambitious 90%-precision target turned out to be
**honestly uncertifiable**, and the gate would rather admit that than fake it. The
certified target below is the real number the data supports.
...
separated correct from incorrect claims well enough to certify *any* precision floor, not
even a lenient one. Rather than invent a number, the gate reports nothing for that family
at all. That refusal is not a bug in the demo; it is the entire thesis, working.
...
keeps landing in the same place no matter how you ask it, and that agreement, plus an
...
Same candidate region, same zero-shot model. Left: one deterministic grounding call,
reported as fact (wrong, on every example shown). Right: the consensus-gated agent's
answer to the identical region, flagged for review rather than asserted.
...
|---|---|---|---|
| Consensus signals alone | 1 *(degenerate)* | n/a | ~0% |
| Consensus + independent segmentation | 0 *(uncertifiable)* | n/a | 0% |
...
- **Cross-run agreement alone is nearly worthless here.** This zero-shot backbone grounds
  *something* in almost every crop it's given, whether or not the something is real, and
  asking it 6 times and finding agreement mostly measures its determinism, not its
  correctness. The deployed weights reflect this (10% each, down from an initial 35%/20%
  equal split).
